...
public class ListCountryTargetPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select * " +
								" from PMI_COUNTRY_TARGET where " +
								" client_sub_segment_code like '%"+search+"%' and YEAR_DATA = '" + currYear + "' order by client_sub_segment_code) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_COUNTRY_TARGET where " +
								" client_sub_segment_code like '%"+search+"%' and YEAR_DATA = '" + currYear + "'";
					}
					...
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select * " +
						" from PMI_COUNTRY_TARGET where year_data = '" + currYear + "'" +
						" order by client_sub_segment_code) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_COUNTRY_TARGET " +
