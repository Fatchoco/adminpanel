...
public class ListSeniorBankerPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select employee_id eid from PMI_SENIOR_BANKER_LIST where EMPLOYEE_ID like '%"+search+"%' order by EMPLOYEE_ID) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_SENIOR_BANKER_LIST where EMPLOYEE_ID like '%"+search+"%'";
					}
					 
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select employee_id eid from PMI_SENIOR_BANKER_LIST order by EMPLOYEE_ID) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_SENIOR_BANKER_LIST ";
