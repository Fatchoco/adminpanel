...
public class ListMISPrdRptPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select PRD_RPT_ID prid, nvl(MIS_PRODUCT_CODE,'-') mis, NVL(REPORT_ID,'-') rpid, " +
								" LABEL_ID lblid, NVL(LABEL,'-') lbl, " +
								" NVL(LABEL_CATEGORY_DESC,'-') catd, LABEL_CATEGORY_ID cat " +
								" from PMI_MIS_PRD_RPT_MAPPING where MIS_PRODUCT_CODE like '%"+search+"%' order by MIS_PRODUCT_CODE) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_MIS_PRD_RPT_MAPPING where MIS_PRODUCT_CODE like '%"+search+"%'";
					}
					...
					else
					{
						...
						query+= "(select PRD_RPT_ID prid, nvl(MIS_PRODUCT_CODE,'-') mis, NVL(REPORT_ID,'-') rpid, " +
							" LABEL_ID lblid, NVL(LABEL,'-') lbl, " +
							" NVL(LABEL_CATEGORY_DESC,'-') catd, LABEL_CATEGORY_ID cat " +
							" from PMI_MIS_PRD_RPT_MAPPING order by MIS_PRODUCT_CODE) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_MIS_PRD_RPT_MAPPING";
