...
public class ListSuperMappingPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select CLIENT_GROUP_ID ctid, nvl(NEW_GROUP_ID,'-') grpid, " +
								" NVL(NEW_GROUP_NAME,'-') grpnm, NVL(NEW_GAM_ID,'-') gamid," +
								" NVL(NEW_GAM_LOCATION,'-') gamloc, NVL(NEW_GROUP_SEGMENT_ID,'-') segid, " +
								" NVL(LINK_TYPE,'-') link, NVL(NEW_GROUP_DOMICILE,'-') gamdom " +
								" from PMI_SUPER_MAPPING where CLIENT_GROUP_ID like '%"+search+"%' order by CLIENT_GROUP_ID) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_SUPER_MAPPING where CLIENT_GROUP_ID like '%"+search+"%'";
					}
					...
					else
					{
						...
						" NVL(NEW_GAM_LOCATION,'-') gamloc, NVL(NEW_GROUP_SEGMENT_ID,'-') segid, " +
						" NVL(LINK_TYPE,'-') link , NVL(NEW_GROUP_DOMICILE,'-') gamdom " +
						" from PMI_SUPER_MAPPING order by CLIENT_GROUP_ID) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_SUPER_MAPPING";
