...
public class ListHeadCountPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select nvl(booking_location_id,'-') bookid, occ_fte fte, occ_rm rm " +
								" from PMI_HEAD_COUNT where " +
						...
						queryCount="select count(*) from PMI_HEAD_COUNT where " +
					...
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select nvl(booking_location_id,'-') bookid, occ_fte fte, occ_rm rm " +
						" from PMI_HEAD_COUNT where PARTITION_KEY like '" + currPar + "%'" +
						" order by booking_location_id) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_HEAD_COUNT " +
