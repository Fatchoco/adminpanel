...
public class ListPrdSegTagPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select PRODUCT_SEGMENT_ID prid,PRODUCT_SEGMENT_TAG1 tag1,PRODUCT_SEGMENT_TAG2 tag2 from PMI_PRD_SEG_TAG_MAPPING where PRODUCT_SEGMENT_ID like '%"+search+"%' order by PRODUCT_SEGMENT_ID) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_PRD_SEG_TAG_MAPPING where PRODUCT_SEGMENT_ID like '%"+search+"%'";
					}
					 
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select PRODUCT_SEGMENT_ID prid,PRODUCT_SEGMENT_TAG1 tag1, PRODUCT_SEGMENT_TAG2 tag2 from PMI_PRD_SEG_TAG_MAPPING order by PRODUCT_SEGMENT_ID) a where rownum<="+end+")";
						query+=" where rnum>"+start;	
						queryCount="select count(*) from PMI_PRD_SEG_TAG_MAPPING";
					}				
