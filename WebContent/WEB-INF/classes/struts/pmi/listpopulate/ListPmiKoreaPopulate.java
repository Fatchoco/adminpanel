...
public class ListPmiKoreaPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select CLIENT_CODE client, BOOKING_LOCATION bookloc, CLIENT_DESC cldesc, " +
								...
								" CLIENT_CODE like '%"+search+"%' and PARTITION_KEY like '" + currPar + "%' order by CLIENT_CODE) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(1) from PMI_KOREA_CLIENT where " +
								" CLIENT_CODE like '%"+search+"%' and PARTITION_KEY like '" + currPar + "%'";
					}
					...
					else
					{
						...
						" TBS_CODE tbs, RTS_CODE rts " +
						" from PMI_KOREA_CLIENT where " +
						" PARTITION_KEY like '" + currPar + "%' order by CLIENT_CODE) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_KOREA_CLIENT " +
