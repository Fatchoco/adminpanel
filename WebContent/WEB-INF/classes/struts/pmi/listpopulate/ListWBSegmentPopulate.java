...
public class ListWBSegmentPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select WB_CLIENT_MEMBER ctid, nvl(RAPM_CLIENT_SUBSEGMENT_ID,'-') rapm from PMI_WB_SEGMENT_MAPPING where WB_CLIENT_MEMBER like '%"+search+"%' order by WB_CLIENT_MEMBER) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_WB_SEGMENT_MAPPING where WB_CLIENT_MEMBER like '%"+search+"%'";
					}
					 
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select WB_CLIENT_MEMBER ctid, nvl(RAPM_CLIENT_SUBSEGMENT_ID,'-') rapm from PMI_WB_SEGMENT_MAPPING order by WB_CLIENT_MEMBER) a where rownum<="+end+")";
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_WB_SEGMENT_MAPPING";
