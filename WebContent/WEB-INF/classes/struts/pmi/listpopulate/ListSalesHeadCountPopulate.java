...
public class ListSalesHeadCountPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select nvl(booking_location_id,'-') bookid, tb_fte fte " +
								" from PMI_HEAD_COUNT_TB where " +
								" booking_location_id like '%"+search+"%' and PARTITION_KEY like '" + currPar + "%' order by booking_location_id) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_HEAD_COUNT_TB where " +
								" booking_location_id like '%"+search+"%' and PARTITION_KEY like '" + currPar + "%'";
					}
					...
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select nvl(booking_location_id,'-') bookid, tb_fte fte " +
						" from PMI_HEAD_COUNT_TB where PARTITION_KEY like '" + currPar + "%'" +
						" order by booking_location_id) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_HEAD_COUNT_TB " +
