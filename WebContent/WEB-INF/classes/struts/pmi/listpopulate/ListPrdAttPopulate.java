...
public class ListPrdAttPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select PRODUCT_ID prid,LIMIT_CATEGORY lmt,BS_TYPE bs from PMI_PRD_ATT_MAPPING where PRODUCT_ID like '%"+search+"%' order by PRODUCT_ID) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_PRD_ATT_MAPPING where PRODUCT_ID like '%"+search+"%'";
					}
					 
					else
					{
						query = "select * from(select a.*, rownum rnum from";
						query+= "(select PRODUCT_ID prid,LIMIT_CATEGORY lmt,BS_TYPE bs from PMI_PRD_ATT_MAPPING order by PRODUCT_ID) a where rownum<="+end+")";
						query+=" where rnum>"+start;	
						queryCount="select count(*) from PMI_PRD_ATT_MAPPING";
					}				
