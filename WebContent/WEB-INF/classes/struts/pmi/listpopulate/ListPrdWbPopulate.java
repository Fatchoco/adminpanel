...
public class ListPrdWbPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select WB_PRODUCT_MEMBER prid, nvl(RAPM_STANDARD_PRODUCT_CODE,'-') rapm from PMI_WB_PRODUCT_MAPPING where WB_PRODUCT_MEMBER like '%"+search+"%' order by WB_PRODUCT_MEMBER) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_WB_PRODUCT_MAPPING where WB_PRODUCT_MEMBER like '%"+search+"%'";
					}
					 
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select WB_PRODUCT_MEMBER prid, nvl(RAPM_STANDARD_PRODUCT_CODE,'-') rapm from PMI_WB_PRODUCT_MAPPING order by WB_PRODUCT_MEMBER) a where rownum<="+end+")";
						query+=" where rnum>"+start;	
						queryCount="select count(*) from PMI_WB_PRODUCT_MAPPING";
					}				
