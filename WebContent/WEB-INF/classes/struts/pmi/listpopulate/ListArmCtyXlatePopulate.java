...
public class ListArmCtyXlatePopulate extends Action {
				...
				try
				{
					 if((request.getParameter("message") != null && request.getParameter("message") != ""))
					{
					 ...
					 if((request.getParameter("Part") != null && request.getParameter("Part") != ""))
						{
								currPar=(request.getParameter("Part"));
						}
					 else {
						   currPar = part2;
					 }
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select ID, BUSINESS_DATE bizd, NVL(COUNTRY_CODE,'-') ctry, NVL(ARM_CD,'-') arm, " +
								" NVL(RM_ID,'-') rm, NVL(RM_NAME,'-') rmn, NVL(CUST_SEGMENT,'-') cseg, NVL(CITY,'-') cty, " +
								" NVL(REGION,'-') reg, NVL(PSGL_CUST_CLASS,'-') psgl, NVL(BIP_CODE,'-') bip  " +
								" from PMI_ARM_CITY_XLATE where " +
								" COUNTRY_CODE like '%"+search+"%' and PARTITION_KEY like '" + currPar + "%' order by COUNTRY_CODE ASC, BUSINESS_DATE DESC) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_ARM_CITY_XLATE where " +
								" COUNTRY_CODE like '%"+search+"%' and PARTITION_KEY like '" + currPar + "%'";
					}
					...
					else
					{
						...
						" NVL(REGION,'-') reg, NVL(PSGL_CUST_CLASS,'-') psgl, NVL(BIP_CODE,'-') bip  " +
						" from PMI_ARM_CITY_XLATE where " +
						" PARTITION_KEY like '" + currPar + "%'" +
						" order by COUNTRY_CODE ASC, BUSINESS_DATE DESC) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_ARM_CITY_XLATE " +
								" where PARTITION_KEY like '" + currPar + "%'";
					...
					request.setAttribute("partition",currPar);
					r.close();
