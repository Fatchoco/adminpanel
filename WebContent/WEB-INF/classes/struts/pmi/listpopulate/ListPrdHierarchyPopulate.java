...
public class ListPrdHierarchyPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select STANDARD_PRODUCT_CODE spc,nvl(STANDARD_PRODUCT_DESC,'-') spd,nvl(PRODUCT_FAMILY_CODE,'-') pfc,nvl(PRODUCT_FAMILY_DESC,'-') pfd,nvl(PRODUCT_SEGMENT_CODE,'-') psc,nvl(PRODUCT_SEGMENT_DESC,'-') psd,nvl(PRODUCT_AREA_CODE,'-') pac,nvl(PRODUCT_AREA_DESC,'-') pad,tb_flag tbf from pmi_product_hierarchy where upper(standard_product_code) like '%"+search+"%' order by standard_product_code)a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from pmi_product_hierarchy where upper(standard_product_code) like '%"+search+"%'";
					}
					 
					else
					{
						query = "select * from(select a.*, rownum rnum from";
						query+= "(select STANDARD_PRODUCT_CODE spc,nvl(STANDARD_PRODUCT_DESC,'-') spd,nvl(PRODUCT_FAMILY_CODE,'-') pfc,nvl(PRODUCT_FAMILY_DESC,'-') pfd,nvl(PRODUCT_SEGMENT_CODE,'-') psc,nvl(PRODUCT_SEGMENT_DESC,'-') psd,nvl(PRODUCT_AREA_CODE,'-') pac,nvl(PRODUCT_AREA_DESC,'-') pad,tb_flag tbf from pmi_product_hierarchy order by standard_product_code)a where rownum<="+end+")";
						query+=" where rnum>"+start;	
						queryCount="select count(*) from pmi_product_hierarchy";
					}				
