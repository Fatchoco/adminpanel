...
public class ListEFXPopulate extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		//initiate db connection
		 ...
		 ResultSet r;
			...
			//System.out.println("tahun" + year2);
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select nvl(booking_location_id,'-') bookid, nvl(Client_id,'-') clid, tmc, nvl(currency,'-') curr " +
								" from PMI_EFX where " +
						...
						queryCount="select count(*) from PMI_EFX where " +
					...
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select nvl(booking_location_id,'-') bookid, nvl(Client_id,'-') clid, tmc, nvl(currency,'-') curr " +
						" from PMI_EFX where PARTITION_KEY like '" + currPar + "%'" +
						" order by booking_location_id) a where rownum<="+end+")"; 
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_EFX " +
