...
public class ListWBCtyPopulate extends Action {
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					 ...
					 if((request.getParameter("Search") != null && request.getParameter("Search") != ""))
					{
						...
						query+= "(select WB_COUNTRY_MEMBER ctid, RAPM_COUNTRY_CODE rapm from PMI_WB_COUNTRY_MAPPING where WB_COUNTRY_MEMBER like '%"+search+"%' order by WB_COUNTRY_MEMBER) a where rownum<="+end+")"; 
						query+= " where rnum>"+start;
						queryCount="select count(*) from PMI_WB_COUNTRY_MAPPING where WB_COUNTRY_MEMBER like '%"+search+"%'";
					}
					 
					else
					{
						query = "select * from (select a.*, rownum rnum from ";
						query+= "(select WB_COUNTRY_MEMBER ctid, RAPM_COUNTRY_CODE rapm from PMI_WB_COUNTRY_MAPPING order by WB_COUNTRY_MEMBER) a where rownum<="+end+")";
						query+=" where rnum>"+start;
						queryCount="select count(*) from PMI_WB_COUNTRY_MAPPING";
