...
public class SalesHeadCountAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		String tableName="PMI_Head_Count_Tb";
		String messageCode1="";
		...
		String readFileStat = "";
		String tabReadStat = "";
		String logFile="SALES_HEAD_COUNT_EXP.txt";
...
try {		
		...
		int fieldNum=3;	
		String HeaderValid="PERIOD\tCOUNTRY\tTB_FTE";	
	...
	if (readFileStat.equals("") && tabReadStat.equals("")) {	
		...
		for(i=0;i<lines.size();i++)
		{
		 ...
		 num=num.replace("\"","");
			...
			else
			{
					...
					//insert to table
					sql="insert into "+tableName+"(partition_key,booking_location_id,tb_fte,CREATE_DATE,CREATE_USER) values(?,?,?,SYSDATE,'PMI_UPLOAD_INSERT')";
		...
		String ctlFile="SALES_HEAD_COUNT";
		...
		if(uploaded_record>0)
		{
			...
			ResultSet r=stmt.executeQuery ("select sum(tb_fte) from pmi_head_count_tb where partition_key='"+partition+"'");
			if(r.next())
			{
				request.setAttribute("TBFTE",r.getBigDecimal(1));
			}
			try
			{
				stmt.close();
