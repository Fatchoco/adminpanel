...
public class EditPrdWb extends Action {
		 ...
		 myConnection = dataSource.getConnection();
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					...
					String sqlUpdate="UPDATE PMI_WB_PRODUCT_MAPPING SET RAPM_STANDARD_PRODUCT_CODE=?,CREATE_DATE=SYSDATE,CREATE_USER='PMI_UPLOAD_UPDATE' where WB_PRODUCT_MEMBER=?";
					...
					catch(SQLException e)
					{
						...
						pstmt.close();
						myConnection.close();
						response.sendRedirect("ListPrdWb.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful "+thisid+" with error"+e);
					}
					myConnection.commit();
					pstmt.close();
					myConnection.close();
					response.sendRedirect("ListPrdWb.scb?Page="+page+"&Search="+search+"&message=Update Successful");
				}
				catch(Exception e)
				{
					System.out.println("ERROR : " + e);
					e.printStackTrace(System.out);
					response.sendRedirect("ListPrdWb.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful: "+e);
				}
