...
public class EditSuperMapping extends Action {
		 ...
		 myConnection = dataSource.getConnection();
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					...
					String sqlUpdate="UPDATE PMI_SUPER_MAPPING SET NEW_GROUP_ID=?,NEW_GROUP_NAME=?," +
							"NEW_GAM_ID=?,NEW_GAM_LOCATION=?,NEW_GROUP_SEGMENT_ID=?,LINK_TYPE=?,NEW_GROUP_DOMICILE=?," +
							"CREATE_DATE=SYSDATE,CREATE_USER='PMI_UPLOAD_UPDATE' where CLIENT_GROUP_ID=?";
					...
					catch(SQLException e)
					{
						...
						pstmt.close();
						myConnection.close();
						response.sendRedirect("ListSuperMapping.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful "+thisid+" with error"+e);
					}
					myConnection.commit();
					pstmt.close();
					myConnection.close();
					response.sendRedirect("ListSuperMapping.scb?Page="+page+"&Search="+search+"&message=Update Successful");
				}
				catch(Exception e)
				{
					System.out.println("ERROR : " + e);
					e.printStackTrace(System.out);
					response.sendRedirect("ListSuperMapping.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful: "+e);
				}
