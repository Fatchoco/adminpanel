...
public class EditMruHierarchy extends Action {
		 ...
		 myConnection = dataSource.getConnection();
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					...
					String sqlUpdate="UPDATE PMI_MRU_HIERARCHY SET MRU=?,TOTAL_MRU=?,CREATE_DATE=SYSDATE,CREATE_USER='PMI_UPLOAD_UPDATE' " +
							" where MRU_CODE=?";
					...
					catch(SQLException e)
					{
						...
						pstmt.close();
						myConnection.close();
						response.sendRedirect("ListMruHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful "+thisid+" with error"+e);
					}
					myConnection.commit();
					pstmt.close();
					myConnection.close();
					response.sendRedirect("ListMruHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Successful");
				}
				catch(Exception e)
				{
					System.out.println("ERROR : " + e);
					e.printStackTrace(System.out);
					response.sendRedirect("ListMruHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful: "+e);
				}
