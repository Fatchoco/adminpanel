...
public class EditSegmentHierarchy extends Action {
		 ...
		 myConnection = dataSource.getConnection();
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					...
					String sqlUpdate="UPDATE PMI_SEGMENT_HIERARCHY SET CLIENT_SUBSEGMENT=?,CLIENT_SEGMENT_ID=?,CLIENT_SEGMENT=?,CLIENT_SEMI_SEGMENT=?,CREATE_DATE=SYSDATE,CREATE_USER='PMI_UPLOAD_UPDATE' where CLIENT_SUBSEGMENT_ID=?";
					...
					catch(SQLException e)
					{
						...
						pstmt.close();
						myConnection.close();
						response.sendRedirect("ListSegmentHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful "+thisid+" with error"+e);
					}
					myConnection.commit();
					pstmt.close();
					myConnection.close();
					response.sendRedirect("ListSegmentHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Successful");
				}
				catch(Exception e)
				{
					System.out.println("ERROR : " + e);
					e.printStackTrace(System.out);
					response.sendRedirect("ListSegmentHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful: "+e);
				}
