...
public class EditCtyHierarchy extends Action {
		 ...
		 myConnection = dataSource.getConnection();
			...
			if(!(request.getSession().getAttribute("username") != null && request.getSession().getAttribute("username") != ""))
			{
				...
				try
				{
					...
					String sqlUpdate="UPDATE PMI_COUNTRY_HIERARCHY SET COUNTRY_NAME=?, REGION_ID=?, " +
							" REGION_NAME=?, TOP_COUNTRY_TAG=?, REGION_CLUSTER=?, CREATE_DATE=SYSDATE,CREATE_USER='PMI_UPLOAD_UPDATE' where COUNTRY_CODE=?";
					...
					catch(SQLException e)
					{
						...
						pstmt.close();
						myConnection.close();
						response.sendRedirect("ListCtyHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful "+thisid+" with error"+e);
					}
					myConnection.commit();
					pstmt.close();
					myConnection.close();
					response.sendRedirect("ListCtyHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Successful");
				}
				catch(Exception e)
				{
					System.out.println("ERROR : " + e);
					e.printStackTrace(System.out);
					response.sendRedirect("ListCtyHierarchy.scb?Page="+page+"&Search="+search+"&message=Update Unsuccessful: "+e);
				}
