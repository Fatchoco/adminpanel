...
public class UploadMasterGeographicProcess extends ActionFormValidated {
	public ActionForward doExecute(ActionMapping mapping, ActionForm form,
		...
		try
		{
			...
			String dbTableName = "MST_GEOGRAPHIC";
			...
			UISummaryFunc.insertLog(summaryDataSource, 1, "nullfornow", "Upload File", "Master Geographic", fileName, "", request.getSession().getAttribute("USER_NAME").toString());
			/* ** End of UI Summary ** */
			...
			String[] fileHeader = {"CITY CODE", "CITY NAME (EN)", "CITY NAME (CN)", "PROVINCE CODE", "PROVINCE NAME (EN)", "PROVINCE NAME (CN)", "REGION CODE", "REGION NAME (EN)", "REGION NAME (CN)", "COUNTRY CODE", "COUNTRY NAME (EN)", "COUNTRY NAME (CN)", "FUNCTION"};
			...
			String logLine = "";
			String query = "INSERT INTO "+dbTableName+"(CITY_CD, CITY, CITY_CN, PROVINCE_CD, PROVINCE, PROVINCE_CN, REGION_CD, REGION, REGION_CN, COUNTRY_CD, COUNTRY, COUNTRY_CN, CREATED_USER) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'TKD_ADM')";
				...
				try
				{
					// Validation
						...
						if(CellValue0.equals(""))
							throw new Exception("City code cannot be blank");
						//Customer code already exists
						for(String t_distcd : vld_distcd)
						{
							if(t_distcd != null && t_distcd.equals(CellValue0))
								throw new Exception("City code duplicate in File");
						}
					...
					//QCheck = "SELECT COUNT(*) roCount FROM MST_GEOGRAPHIC WHERE CITY_CD = '"+CellValue0+"'";
					...
					//Delete Record
					stmt			= myConnection.createStatement();
					stmt.executeUpdate("DELETE FROM MST_GEOGRAPHIC WHERE CITY_CD = '"+CellValue0+"'");
					stmt.execute("COMMIT");
					stmt.close();
			...
			//LogFiles
			String logFileName = "MasterGeographic_RejectedRecord.xls";
