...
public class UploadMasterDistributorProcess extends ActionFormValidated {
	public ActionForward doExecute(ActionMapping mapping, ActionForm form,
		...
		try
		{
			...
			String dbTableName = "MST_DISTRIBUTOR";
			...
			UISummaryFunc.insertLog(summaryDataSource, 1, "nullfornow", "Upload File", "Master Distributor", fileName, "", request.getSession().getAttribute("USER_NAME").toString());
			/* ** End of UI Summary ** */
			...
			String[] fileHeader = {"DISTRIBUTOR CODE", "DISTRIBUTOR NAME (EN)", "DISTRIBUTOR NAME (CN)", "DISTRIBUTOR ADDRESS (EN)", "DISTRIBUTOR ADDRESS (CN)", "DISTRIBUTOR POSTAL CODE (EN)", "DISTRIBUTOR POSTAL CODE (CN)", "CITY CODE", "LEVEL", "PARENT DISTRIBUTOR CODE", "FUNCTION"};
			...
			String logLine = "";
			String query = "INSERT INTO "+dbTableName+"(DISTRIBUTOR_CD, DISTRIBUTOR_NM, DISTRIBUTOR_NM_CN, DISTRIBUTOR_ADDRESS, DISTRIBUTOR_ADDRESS_CN, DISTRIBUTOR_POSTAL_CD, DISTRIBUTOR_POSTAL_CD_CN, CITY_CD, LEVELS, PARENT_DISTRIBUTOR_CD, CREATED_USER) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'TKD_ADM')";
			...
			for(int x = 1; x < dataHolder.size(); x++)
			{
				...
				try
				{
					...
					//QCheck = "SELECT COUNT(*) roCount FROM MST_DISTRIBUTOR WHERE DISTRIBUTOR_CD = '"+CellValue0+"'";
					...
					//Delete Record
					stmt			= myConnection.createStatement();
					stmt.executeUpdate("DELETE FROM MST_DISTRIBUTOR WHERE DISTRIBUTOR_CD = '"+CellValue0+"'");
					stmt.execute("COMMIT");
					stmt.close();
					
					String VarTemp = (myCell10==null)?"":cleanColumn.execute(myCell10.toString());
				...
				{
					logLine = CellValue0+'\t'+CellValue1+'\t'+CellValue2+'\t'+CellValue3+'\t'+CellValue4+'\t'+CellValue5+'\t'+CellValue6+'\t'+CellValue7+'\t'+CellValue8+'\t'+CellValue9+'\t'+eins.getMessage().trim();
			...
			//LogFiles
			String logFileName = "MasterDistributor_RejectedRecord.xls";
