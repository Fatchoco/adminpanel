...
public class UploadMappingTakedaCategoryInitial extends ActionFormValidated {
	public ActionForward doExecute(ActionMapping mapping, ActionForm form,
		...
		{
			//Get Server Realpath
			String logName = "MappingTakedaCategory_RejectedRecord.xls";
			...
			qdata = "SELECT trim(to_char(to_date(MAX(CURRENT_PERIOD),'RRRRMMDD'),'Month')) || ' ' || trim(to_char(to_date(MAX(CURRENT_PERIOD),'RRRRMMDD'),'YYYY'))  CURRENT_PERIOD FROM PAR_SYSTEM";
			rset = stmt.executeQuery(qdata);
			rset.next();
			result = rset.getString("CURRENT_PERIOD");
			rset.close();
			
			qdata = "SELECT * FROM COUNTRY";
			rset = stmt.executeQuery(qdata);
			cn_result = ResultSupport.toResult(rset);
			rset.close();
			stmt.close();
			myConnection.close();
