...
import struts.tkd.actfrm.FormUploadPagePeriod;
...
public class UploadDistributorAdjustmentFilesProcess extends ActionFormValidated {
	public ActionForward doExecute(ActionMapping mapping, ActionForm form,
		...
		try
		{
			...
			FormUploadPagePeriod myForm = (FormUploadPagePeriod) form;
			...
			String zipFileTrg = filePath + "extractDistAdjFiles";
			try
			{
...
//				throw new Exception(r_input+"::"+r_error+"::("+"unzip -o "+zipFileSrc+" -d "+zipFileTrg+""+")");
				...
				current_period = myForm.getCur_period();
		    ...
		    for(File file:files)
		    {
		    	...
		    	if(file.isFile())
		    	{
		    		try
		    		{
		    			...
		    			UISummaryFunc.insertLog(summaryDataSource, 1, "nullfornow", "Upload File", "Distributor Adjustment File", file.getName(), "Adjustment Period = "+current_period, request.getSession().getAttribute("USER_NAME").toString());
		    			/* ** End of UI Summary ** */
				        ...
				        dataHolder = excelFunc.getExcelContent(file.getParent()+file.separator, file.getName());
							...
							stmt.execute("DELETE "+dbTableName+" WHERE FILE_NAME = '"+excelFunc.removeDistFileNamePeriod(file.getName())+"' AND PERIOD = '"+current_period+"' AND FLAG_ADJUSTMENT = 'Y'");
							stmt.close();
							myConnection.close();
						...
						String logLine = "";
						String query = "INSERT INTO "+dbTableName+"(RID,FILE_NAME,C1,C2,C3,C4,C5,C6,C7,C8,C9,C10,C11,C12,C13,C14,C15,C16,C17,C18,C19,C20,C21,C22,C23,C24,C25,C26,C27,C28,C29,C30,C31,C32,C33,C34,C35,C36,C37,C38,C39,C40,C41,C42,C43,C44,C45,C46,C47,C48,C49,C50,PERIOD,CREATED_USER,FLAG_ADJUSTMENT) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'TKD_ADM', 'Y')";
			...
			//LogFiles
			String logFileName = "Distributor_Adjustment_Files_Exception.xls";
			...
			if(true)	//Mau ilangin if nya tapi malas jadinya set true dulu
			{
				...
				myConnection	= dataSource.getConnection();
				stmt			= myConnection.createStatement();
				rset			= stmt.executeQuery("SELECT * FROM EXCP_DATA_STG1 WHERE PERIOD = '"+current_period+"' AND FLAG_ADJUSTMENT = 'Y' ORDER BY FILE_NAME, RID");
				logRowIter = 1;
				while(rset.next())
				{
					logRow = logSheet.createRow(logRowIter);
			...
			request.setAttribute("adj_period", current_period);
