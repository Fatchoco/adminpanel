...
function NavigatePage()
{
	 ...
	 window.location.href="ViewMappingProduct.tkd?Page="+currentPage+"&Search="+searchQ;
}
function Search()
{
	 var searchQ=document.getElementById("txtSearch").value;
	 window.location.href="ViewMappingProduct.tkd?Search="+searchQ;
}
function Export()
{
	 var searchQ=document.getElementById("txtSearch").value;
	 window.location.href="ExportMappingProduct.tkd?Search="+searchQ;
}
...
function Delete()
{
	 ...
	 if (r==true)
	 {
		 window.location.href="DeleteMappingProduct.tkd";
	 }
}
...
<title>View Mapping Product - Takeda</title>
...
<div class="content">
	...
	<b><u>Product Mapping - Result Page</u></b>
	</p>
	<table border="0" style="font-size: 15px;">
		...
		<tr>
			<td colspan="3"><c:out value="${deletecomment}" />
		...
		<tr><td>
			<table border="0" cellspacing="0" cellpadding="0" class="data">
				...
				<tr>
					...
					<th scope="col">DISTRIBUTOR PRODUCT CODE</th>
					<th scope="col">DISTRIBUTOR PRODUCT NAME (CN)</th>
					<th scope="col">TAKEDA PRODUCT CODE</th>
					<th scope="col">TAKEDA PRODUCT NAME (CN)</th>
				</tr>
				<c:forEach var="item" items="${result.rows}">
					<c:set var="i" value="${i+1}" />
					<tr id='tr_<c:out value="${item.distributor_product_cd}" />' <c:if test="${i % 2 == 1}"> class="oddRow" </c:if>
						...
						<td>
						<c:out value="${item.distributor_product_cd}" />
						<c:if test="${item.distributor_product_cd == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						<c:out value="${item.distributor_product_nm}" />
						<c:if test="${item.distributor_product_nm == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
		...
		<tr><td>
			<table border="0" align="right" cellpadding="0" cellspacing="0" class="tkd_button">
				<tr>
		            ...
		            <td class="tkd_button_right">&nbsp;</td>
		            <td class="tkd_button_left">&nbsp;</td>
		            <td align="center" class="tkd_button_middle"><a href="#" onclick="javascript:window.location.href='UploadMappingProduct.tkd'">UPLOAD PAGE</a></td>
		            <td class="tkd_button_right">&nbsp;</td>
		            <td class="tkd_button_left">&nbsp;</td>
