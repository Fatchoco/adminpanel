...
<script language="javascript">
	function Import()
	{
		...
		var cty=document.forms[0].country_cod.value;
		var ncty=cty;
		if(cty=="CN") ncty="CHINA";
		if(cty=="PH") ncty="PHILIPPHINES";
		if(fileFullPath=="")
		{
		alert("Please select the file to upload first");
		}
		else if(fileFullPath.substring(0,20)!="MAP_TAKEDA_CATEGORY_"){
			alert("Wrong file name");
		}
		else if(fileFullPath.substring(20,25)!=ncty){
			alert("Wrong country file");
		}
		else
		{
...
<title>Upload Mapping Takeda Category - Takeda</title>
<body>
<div class="content">
	<script type="text/javascript" src="wz_tooltip.js"></script>
	<c:import url="Header2.tkd"></c:import>
	<html:form action="/UploadMappingTakedaCategoryProcess" method="post" enctype="multipart/form-data">
		<p class="txtTitleContent" align="left">
		<b><u>Mapping Takeda Category Upload</u></b>
		</p>
		<table border="0">
			<tr>
				<td colspan="3">
			...
			<tr>
				<td colspan="3">
					Country : 
					<html-el:select property="country_cod" value="CN">
			...
			<tr align="right">
				...
				<td colspan="3">
					...
					<c:if test="${(logfile == 1) && (upload_record == null)}">
						<table border="0" cellpadding="0" cellspacing="0" class="tkd_button">
							<tr>
					            ...
					            <td align="center" class="tkd_button_middle"><a href="#" onclick="window.open('ExportLogMappingProduct.tkd')">LAST LOG</a></td>
					...
					<table border="0" cellpadding="0" cellspacing="0" class="tkd_button">
						<tr>
							<td class="tkd_button_left">&nbsp;</td>
				            <td align="center" class="tkd_button_middle"><a href="#" onclick="javascript:window.location.href='ViewMappingTakedaCategory.tkd'">RESULT PAGE</a></td>
				            <td class="tkd_button_right">&nbsp;</td>
						</tr>
					</table>
					</div>
	...
	<c:if test="${deleted_record > 0}">
		Successful data deleted : <c:out value="${deleted_record}" /> records <br/>
	</c:if>
		<c:if test="${reject_record > 0}">
			Rejected data from the last upload : <c:out value="${reject_record}" /> records <a onclick="window.open('ExportLogMappingTakedaCategory.tkd')" style="cursor: hand;cursor: pointer;">(Click here to see the rejected records)</a>
		</c:if>
	</c:if>
