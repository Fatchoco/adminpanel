...
<title>Upload Mapping MR - Takeda</title>
<body>
<div class="content">
	<script type="text/javascript" src="wz_tooltip.js"></script>
	<c:import url="Header.tkd"></c:import>
	<html:form action="/UploadMappingMRProcess" method="post" enctype="multipart/form-data">
		<p class="txtTitleContent" align="left">
		<b><u>MR Mapping Upload</u></b>
		</p>
			...
			<tr align="right">
				...
				<td colspan="3">
					...
					<c:if test="${(logfile == 1) && (upload_record == null)}">
						<table border="0" cellpadding="0" cellspacing="0" class="tkd_button">
							<tr>
					            ...
					            <td align="center" class="tkd_button_middle"><a href="#" onclick="window.open('ExportLogMappingMR.tkd')">LAST LOG</a></td>
					...
					<table border="0" cellpadding="0" cellspacing="0" class="tkd_button">
						<tr>
							<td class="tkd_button_left">&nbsp;</td>
				            <td align="center" class="tkd_button_middle"><a href="#" onclick="javascript:window.location.href='ViewMappingMR.tkd'">RESULT PAGE</a></td>
				            <td class="tkd_button_right">&nbsp;</td>
						</tr>
					</table>
					</div>
	...
	<c:if test="${upload_record != null}">
		...
		<c:if test="${reject_record > 0}">
			Rejected data from the last upload : <c:out value="${reject_record}" /> records <a onclick="window.open('ExportLogMappingMR.tkd')" style="cursor: hand;cursor: pointer;">(Click here to see the rejected records)</a>
		</c:if>
	</c:if>
