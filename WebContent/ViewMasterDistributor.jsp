...
function NavigatePage()
{
	 ...
	 window.location.href="ViewMasterDistributor.tkd?Page="+currentPage+"&Search="+searchQ;
}
function Search()
{
	 var searchQ=document.getElementById("txtSearch").value;
	 window.location.href="ViewMasterDistributor.tkd?Search="+searchQ;
}
function Export()
{
	 var searchQ=document.getElementById("txtSearch").value;
	 window.location.href="ExportMasterDistributor.tkd?Search="+searchQ;
}
...
function Delete()
{
	 ...
	 if (r==true)
	 {
		 window.location.href="DeleteMasterDistributor.tkd";
	 }
}
...
<title>View Master Distributor - Takeda</title>
...
<div class="content">
	...
	<b><u>Distributor Master - Result Page</u></b>
	</p>
		...
		<tr><td>
			<table border="0" cellspacing="0" cellpadding="0" class="data">
				...
				<tr>
					...
					<th scope="col">DISTRIBUTOR NAME (EN)</th>
					<th scope="col">DISTRIBUTOR NAME (CN)</th>
					<th scope="col">DISTRIBUTOR ADDRESS (EN)</th>
					<th scope="col">DISTRIBUTOR ADDRESS (CN)</th>
					<th scope="col">DISTRIBUTOR POSTAL CODE (EN)</th>
					<th scope="col">DISTRIBUTOR POSTAL CODE (CN)</th>
					<th scope="col">CITY CODE</th>
					<th scope="col">LEVEL</th>
					<th scope="col">PARENT DISTRIBUTOR CODE</th>
					<!--<td>
					<c:out value="PERIOD" />
					</td>
					<td>
				...
				<c:forEach var="item" items="${result.rows}">
					...
					<tr id='tr_<c:out value="${item.distributor_cd}" />' <c:if test="${i % 2 == 1}"> class="oddRow" </c:if>
						...
						<td>
						<c:out value="${item.distributor_nm}" />
						<c:if test="${item.distributor_nm == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						<c:out value="${item.distributor_nm_cn}" />
						<c:if test="${item.distributor_nm_cn == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						<c:out value="${item.distributor_address}" />
						<c:if test="${item.distributor_address == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						<c:out value="${item.distributor_address_cn}" />
						<c:if test="${item.distributor_address_cn == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						<c:out value="${item.distributor_postal_cd}" />
						<c:if test="${item.distributor_postal_cd == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						<c:out value="${item.distributor_postal_cd_cn}" />
						<c:if test="${item.distributor_postal_cd_cn == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						...
						<c:if test="${item.levels == null}">
							&nbsp;
						</c:if>
						</td>
						<td>
						<c:out value="${item.parent_distributor_cd}" />
						<c:if test="${item.parent_distributor_cd == null}">
							&nbsp;
						</c:if>
						</td>
						<!--<td>
		...
		<tr><td>
			<table border="0" align="right" cellpadding="0" cellspacing="0" class="tkd_button">
				<tr>
		            ...
		            <td class="tkd_button_right">&nbsp;</td>
					<td class="tkd_button_left">&nbsp;</td>
		            <td align="center" class="tkd_button_middle"><a href="#" onclick="javascript:window.location.href='UploadMasterDistributor.tkd'">UPLOAD PAGE</a></td>
		            <td class="tkd_button_right">&nbsp;</td>
		            <td class="tkd_button_left">&nbsp;</td>
